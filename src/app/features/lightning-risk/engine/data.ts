...
export interface ValueTableRow extends TableRowBase {
  value: number;
}

export interface StructureTypeRow extends TableRowBase {
  LT: number;
  LF: number;
  LO: number;
}

export interface ServiceTypeRow extends TableRowBase {
  LF: number;
  LO: number;
}
...
export interface StandardRiskThresholds {
  RT1: number;
  RT2: number;
  RT3: number;
}
...
export interface LightningTables {
  specialHazard: readonly ValueTableRow[];
  protectionLevel: readonly ValueTableRow[];
  spd: readonly ValueTableRow[];
  location: readonly ValueTableRow[];
  fireRisk: readonly ValueTableRow[];
  floorType: readonly ValueTableRow[];
  fireProvisions: readonly ValueTableRow[];
  structureType: readonly StructureTypeRow[];
  serviceType: readonly ServiceTypeRow[];
  cultureHeritageLF: number;
}

// Edition metadata surfaced in the UI (plan v2, section 6)
export const STANDARD_EDITION: StandardEdition = {
  id: 'iec-62305-2-2010',
...
export const ACTIVE_STANDARD = STANDARDS[STANDARD_EDITION.id] as StandardDefinition;
export const RT = ACTIVE_STANDARD.RT;

// ============================================================
// Lookup tables. Each entry: { label, value, ... } with source
// and verification status documented. lookupValue() reads .value.
// ============================================================
export const TABLES: LightningTables = {
  ...
  fireProvisions: [
    ...
    {
      label: 'Extinguishers / manual alarm / hydrants / fire compartments / escape routes',
      value: 0.5,
      source: 'Table C.3',
      status: 'VERIFIED',
    },
    {
      label: 'Automatic extinguishing or alarm (with overvoltage protection, < 10 min response)',
      value: 0.2,
      source: 'Table C.3',
      status: 'VERIFIED',
    },
...
export type SpecialHazardLabel = (typeof TABLES.specialHazard)[number]['label'];
export type ProtectionLevelLabel = (typeof TABLES.protectionLevel)[number]['label'];
export type SpdLabel = (typeof TABLES.spd)[number]['label'];
export type LocationLabel = (typeof TABLES.location)[number]['label'];
export type FireRiskLabel = (typeof TABLES.fireRisk)[number]['label'];
export type FloorTypeLabel = (typeof TABLES.floorType)[number]['label'];
export type FireProvisionsLabel = (typeof TABLES.fireProvisions)[number]['label'];
export type StructureTypeLabel = (typeof TABLES.structureType)[number]['label'];
export type ServiceTypeLabel = (typeof TABLES.serviceType)[number]['label'];
export type ElectricalLineLabel = 'No' | 'Aerial' | 'Underground';

// ---------- specialized accessors for the multi-column tables ----------
export function lookupValue(tableName: LookupTableName, label: string): number | null {
  const row = TABLES[tableName].find((entry) => entry.label === label);
  return row ? row.value : null;
}

export function structureLoss(label: string): Pick<StructureTypeRow, 'LT' | 'LF' | 'LO'> | null {
  const row = TABLES.structureType.find((entry) => entry.label === label);
  return row ? { LT: row.LT, LF: row.LF, LO: row.LO } : null;
}

export function serviceLoss(label: string): Pick<ServiceTypeRow, 'LF' | 'LO'> | null {
  const row = TABLES.serviceType.find((entry) => entry.label === label);
...
export const ZONE_RATIO_NZ_NT = 1; // nz/nt: single zone holds all persons/users
export const ZONE_TIME_TZ_8760 = 1; // tz/8760: continuous occupancy all year
...
export const PTA_DEFAULT = 1; // VERIFIED (Table B.2) - no protection measures
...
export const PMS_DEFAULT = 1; // NEEDS_VERIFICATION (Table B.5)
...
export const LIFE_CRITICAL_LO_FALLBACK = 0.001; // NEEDS_VERIFICATION (Table C.2)

// The standard's typical loss for L3 (cultural heritage), Table C.4.
export const CULTURE_LF = TABLES.cultureHeritageLF; // VERIFIED (Table C.4)

// A quick audit summary for grepping
export const DATA_AUDIT: readonly (readonly [string, string, VerificationStatus])[] = [
...
export interface LightningDefaults {
...
// ============================================================
export const LIGHTNING_DEFAULTS: LightningDefaults = {
  project: { name: '', id: '' },
  Ng: 8,
  L: '',
  W: '',
...
export const KTC_EXAMPLE: LightningDefaults = {
