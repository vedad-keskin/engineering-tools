/* ===================================================================
   CAPACITY PLANNER - application logic
   =================================================================== */
...
let collapsedSummaryGroups = new Set();
let collapsedScenGroups = new Set();
let networkFilters = { search: '', type: '', system: '', status: '', voltage: '', primary: '', redundant: '', feed: '', rating: '', connloads: '', connkw: '', demandkva: '', pf: '', util: '' };
let summaryFilters = { type: '', tag: '', name: '', rating: '', sizing: '', active: '', util: '' };
...
let summarySort = null;         // { key, dir } for summary equipment sort
...
const SUMMARY_COLUMNS = [
  { key: 'type', label: 'Type' }, { key: 'tag', label: 'Tag' }, { key: 'name', label: 'Name' },
  { key: 'rating', label: 'Rating' }, { key: 'sizing', label: 'Sizing kVA' },
  { key: 'active', label: 'Active kVA' }, { key: 'util', label: 'Utilisation' },
];
...
function currentScenario() {
  if (state.activeScenarioId == null) return null;
...
function renderActive() {
  document.querySelector('main').classList.toggle('wide',
    ['network', 'schedule', 'scenarios', 'summary'].indexOf(activeView) >= 0);
  ...
  if (activeView === 'scenarios') renderScenarios();
...
function renderSettings() {
  ...
  const order = ['utility', 'mv-dist', 'transformer', 'generator', 'ups', 'dist', 'load'];
  order.forEach(t => {
...
function deleteNode(id) {
  ...
  showConfirm(msg, 'Delete', () => {
    ...
    state.scenarios.forEach(s => { subtree.forEach(i => { delete s.overrides[i]; }); });
...
function deleteSelectedNodes(idSet, typeFilter) {
  ...
  showConfirm('Delete ' + targets.length + ' selected item' + (targets.length === 1 ? '' : 's') +
    ...
    state.scenarios.forEach(s => { doomed.forEach(i => { delete s.overrides[i]; }); });
...
$('#btnLoadExample').addEventListener('click', () => {
  ...
  collapsedSummaryGroups = new Set();
  collapsedScenGroups = new Set();
  ...
  summaryFilters = { type: '', tag: '', name: '', rating: '', sizing: '', active: '', util: '' };
  netSort = null; schedSort = null; summarySort = null; netGroupBy = 'none';
  renderActive();
...
$('#btnSummaryColumns').addEventListener('click', (e) => showColumnMenu(e, 'summary', SUMMARY_COLUMNS));
...
function activeField(node) {
  const scen = currentScenario();
  const override = scen ? scen.overrides[node.id] : null;
  const isBase = scen ? override == null : true;
  const baseOn = node.active !== false;
  const on = override != null ? (override === 'active') : baseOn;
  ...
  inp.addEventListener('change', () => {
    const val = inp.checked ? 'active' : 'inactive';
    if (scen) scen.overrides[node.id] = val; else node.active = (val === 'active');
    renderModal();
  });
  ...
  if (scen && !isBase) {
    wrap.appendChild(el('div', { class: 'hint' }, ['Override in "' + scen.name + '": ' + (on ? 'Active' : 'Inactive') + ' (base: ' + (baseOn ? 'Active' : 'Inactive') + ')']));
  }
  return wrap;
}
...
function updateSelectionUI() {
  ...
  const sels = $('#schedBody').children.length;
...
/* ===================================================================
   OPERATING SCENARIOS VIEW
   =================================================================== */
function renderScenarios() {
  const head = $('#scenariosMatrixHead'); const body = $('#scenariosMatrixBody');
  if (!head || !body) return;
  head.innerHTML = ''; body.innerHTML = '';
  const cols = [{ id: null, name: 'Normal', base: true }].concat(state.scenarios);
  head.appendChild(el('th', {}, ['Item']));
  cols.forEach(col => {
    const th = el('th', { class: 'scen-col' });
    const wrap = el('div', { class: 'scen-head' });
    const isActive = (col.id == null && state.activeScenarioId == null) || (col.id != null && state.activeScenarioId === col.id);
    if (isActive) wrap.appendChild(el('span', { class: 'badge ok', style: 'align-self:center;' }, ['Active']));
    else {
      const act = el('button', { class: 'tbtn primary', style: 'justify-content:center;' }, ['Activate']);
      act.addEventListener('click', () => { state.activeScenarioId = col.id; renderActive(); showToast('Switched to "' + (col.id == null ? 'Normal' : (col.name || 'Untitled')) + '".', 'success'); });
      wrap.appendChild(act);
    }
    if (col.base) wrap.appendChild(el('span', { style: 'font-weight:700;font-size:12px;text-align:center;' }, ['Normal (base)']));
    else {
      const s = state.scenarios.find(x => x.id === col.id);
      const nameIn = el('input', { type: 'text', value: s.name });
      nameIn.addEventListener('input', () => { s.name = nameIn.value || 'Untitled'; renderScenarioSelect(); });
      wrap.appendChild(nameIn);
      const acts = el('div', { class: 'scen-actions' });
      acts.appendChild(copyIconBtn('Duplicate scenario', () => duplicateScenario(s.id)));
      const del = el('button', { class: 'iconbtn danger', title: 'Delete scenario' }, ['✕']);
      del.addEventListener('click', (e) => { e.stopPropagation(); deleteScenario(s.id); });
      acts.appendChild(del);
      wrap.appendChild(acts);
    }
    th.appendChild(wrap); head.appendChild(th);
  });
  function scenarioRow(n) {
    const tr = el('tr', {});
    const itemTd = el('td', {});
    const badge = el('span', { class: 'node-badge' }, [TYPE_META[n.type].short]);
    badge.style.background = voltageColor(nodeVoltageV(n));
    itemTd.appendChild(badge);
    itemTd.appendChild(document.createTextNode(' ' + label(n)));
    tr.appendChild(itemTd);
    cols.forEach(col => {
      const td = el('td', { class: 'scen-cell' });
      const scen = col.id == null ? null : state.scenarios.find(x => x.id === col.id);
      const baseOn = n.active !== false;
      const override = scen ? scen.overrides[n.id] : null;
      const on = override != null ? (override === 'active') : baseOn;
      const row = el('label', { class: 'switch-row' });
      const inp = el('input', { type: 'checkbox' });
      inp.checked = on;
      inp.addEventListener('change', () => {
        if (scen) {
          if ((inp.checked ? 'active' : 'inactive') === (baseOn ? 'active' : 'inactive')) delete scen.overrides[n.id];
          else scen.overrides[n.id] = inp.checked ? 'active' : 'inactive';
        } else n.active = inp.checked;
        renderActive();
      });
  ...
  // Group by system (like the Network tab): systems in creation order, then Unassigned.
  const groups = [];
  state.systems.forEach(s => {
    const comps = state.nodes.filter(n => n.systemId === s.id);
    if (comps.length) groups.push({ label: s.name, color: s.color, key: 's' + s.id, components: comps });
  });
  const unassigned = state.nodes.filter(n => n.systemId == null);
  if (unassigned.length) groups.push({ label: 'Unassigned to system', color: '#5b6675', key: 'unassigned', components: unassigned });

  groups.forEach(g => {
    const collapsed = collapsedScenGroups.has(g.key);
    const head = el('tr', { class: 'system-group-header' });
    const headTd = el('td', { colspan: 1 + cols.length });
    const caret = el('span', { class: 'collapse-caret' }, [collapsed ? '\u25B8' : '\u25BE']);
    const sw = el('span', { class: 'vl-swatch' }); sw.style.background = g.color;
    const nameSpan = el('span', { style: 'font-weight:800;font-size:12px;letter-spacing:.4px;text-transform:uppercase;padding-left:8px;' }, [g.label]);
    caret.addEventListener('click', (e) => { e.stopPropagation(); if (collapsedScenGroups.has(g.key)) collapsedScenGroups.delete(g.key); else collapsedScenGroups.add(g.key); renderScenarios(); });
    headTd.appendChild(caret); headTd.appendChild(sw); headTd.appendChild(nameSpan);
    head.addEventListener('click', () => { if (collapsedScenGroups.has(g.key)) collapsedScenGroups.delete(g.key); else collapsedScenGroups.add(g.key); renderScenarios(); });
    head.appendChild(headTd);
    body.appendChild(head);
    if (collapsed) return;
    g.components.slice().sort((a, b) => ((a.order == null ? 1e9 : a.order) - (b.order == null ? 1e9 : b.order)) || (a.id - b.id))
      .forEach(n => body.appendChild(scenarioRow(n)));
  });
}
function renderScenarioSelect() {
  const sel = $('#activeScenarioSelect'); if (!sel) return;
  sel.innerHTML = '';
  sel.appendChild(el('option', { value: '' }, ['Normal']));
  state.scenarios.forEach(s => sel.appendChild(el('option', { value: String(s.id) }, [s.name || 'Untitled'])));
  sel.value = state.activeScenarioId == null ? '' : String(state.activeScenarioId);
}
$('#activeScenarioSelect').addEventListener('change', (e) => {
  state.activeScenarioId = e.target.value === '' ? null : parseInt(e.target.value, 10);
  renderActive();
});
function duplicateScenario(id) {
  const src = state.scenarios.find(s => s.id === id);
  if (!src) return;
  const copy = { id: state.nextScenarioId++, name: src.name + ' (copy)', overrides: JSON.parse(JSON.stringify(src.overrides || {})) };
...
function deleteScenario(id) {
  ...
  if (!s) return;
  showConfirm('Delete scenario "' + (s.name || 'Untitled') + '"? Component status overrides for it will be removed.', 'Delete', () => {
    state.scenarios = state.scenarios.filter(x => x.id !== id);
    if (state.activeScenarioId === id) state.activeScenarioId = null;
    renderActive();
...
function createScenario() {
  ...
  state.scenarios.push({ id, name, overrides: {} });
  state.activeScenarioId = id;
  renderActive();
  showToast('Created and activated "' + name + '". In this scenario you can now toggle component Active/Inactive without affecting Normal mode.', 'success');
}
$('#btnNewScenario').addEventListener('click', createScenario);

/* ===================================================================
   SUMMARY VIEW
   =================================================================== */
function buildSummaryFilterRow() {
  const row = $('#summaryColFilters'); if (!row) return;
  row.innerHTML = '';
  const setF = (k) => (v) => { summaryFilters[k] = v; withFocusPreserved('#summaryColFilters', renderSummary); };
  row.appendChild(makeFilterSelect('type', summaryFilters.type, Object.keys(TYPE_META).map(t => [t, TYPE_META[t].label]), setF('type'), 'sumF.type'));
  row.appendChild(makeFilterText('tag', summaryFilters.tag, 'Tag', setF('tag'), 'sumF.tag'));
  row.appendChild(makeFilterText('name', summaryFilters.name, 'Name', setF('name'), 'sumF.name'));
  row.appendChild(makeFilterText('rating', summaryFilters.rating, 'Rating', setF('rating'), 'sumF.rating'));
  row.appendChild(makeFilterText('sizing', summaryFilters.sizing, 'Sizing', setF('sizing'), 'sumF.sizing'));
  row.appendChild(makeFilterText('active', summaryFilters.active, 'Active', setF('active'), 'sumF.active'));
  row.appendChild(makeFilterText('util', summaryFilters.util, '%', setF('util'), 'sumF.util'));
}

const SUMMARY_SORT_KEYS = {
  type: { get: n => TYPE_META[n.type].label, numeric: false },
  tag: { get: n => (n.tag || '').toLowerCase(), numeric: false },
  name: { get: n => (n.name || '').toLowerCase(), numeric: false },
  rating: { get: n => nodeRatingKVA(n) || 0, numeric: true },
  sizing: { get: n => nodeRollup(state.nodes, n, currentScenario()).sizingKVA || 0, numeric: true },
  active: { get: n => nodeRollup(state.nodes, n, currentScenario()).activeKVA || 0, numeric: true },
  util: { get: n => nodeRollup(state.nodes, n, currentScenario()).utilizationPct || 0, numeric: true },
};

function summarySortCompare(a, b) {
  if (!summarySort) return 0;
  const mk = SUMMARY_SORT_KEYS[summarySort.key];
  if (!mk) return 0;
  const av = mk.get(a), bv = mk.get(b);
...
function applySummarySort(key) {
  if (!SUMMARY_SORT_KEYS[key]) return;
  if (!summarySort || summarySort.key !== key) summarySort = { key, dir: 'asc' };
  else if (summarySort.dir === 'asc') summarySort.dir = 'desc';
  else summarySort = null;
  renderSummary();
}

function attachSummarySort() {
  const ths = $all('#equipUtilTable thead tr:first-child th');
  const map = { 0: 'type', 1: 'tag', 2: 'name', 3: 'rating', 4: 'sizing', 5: 'active', 6: 'util' };
  ths.forEach((th, i) => {
    if (!map[i]) return;
    th.style.cursor = 'pointer';
    th.title = 'Click to sort';
    th.addEventListener('click', () => applySummarySort(map[i]));
  });
}

function renderSummary() {
  const scen = currentScenario();
  const t = systemTotals(state.nodes, scen);
  metricStrip('#summaryMetrics', [
    ['Connected Load (kW)', fmt(t.connectedKW, 1)],
    ['Connected Load (kVA)', fmt(t.connectedKVA, 1)],
    ['Demand (kW)', fmt(t.demandKW, 1), true],
    ['Demand (kVA)', fmt(t.demandKVA, 1), true],
  ]);
  metricStrip('#categoryMetrics', [
    ['Critical demand (kW / kVA)', fmt(t.cat.critical.kW, 1) + ' / ' + fmt(t.cat.critical.kVA, 1)],
    ['Essential demand (kW / kVA)', fmt(t.cat.essential.kW, 1) + ' / ' + fmt(t.cat.essential.kVA, 1)],
    ['Non-essential demand (kW / kVA)', fmt(t.cat['non-essential'].kW, 1) + ' / ' + fmt(t.cat['non-essential'].kVA, 1)],
  ]);
  metricStrip('#utilityMetrics', [
    ['Utility Demand (kVA)', fmt(t.utilityActive, 1)],
    ['Utility Contract (kVA)', fmt(t.utilityContract, 0)],
    ['Utility Utilisation', t.utilityPct != null ? fmt(t.utilityPct, 0) + '%' : '\u2014', true],
  ]);

  const allEquip = state.nodes.filter(n => n.type !== 'load');
  const hasAnyRating = allEquip.some(n => nodeRatingKVA(n) != null);
  $('#equipEmpty').style.display = hasAnyRating ? 'none' : 'block';
  $('#equipUtilTable').style.display = hasAnyRating ? 'table' : 'none';

  // Build filter row
  buildSummaryFilterRow();

  // Filter
  let equipRows = allEquip.filter(n => {
    if (summaryFilters.type && n.type !== summaryFilters.type) return false;
    const r = nodeRollup(state.nodes, n, scen);
    if (summaryFilters.tag && !(n.tag || '').toLowerCase().includes(summaryFilters.tag.toLowerCase())) return false;
    if (summaryFilters.name && !(n.name || '').toLowerCase().includes(summaryFilters.name.toLowerCase())) return false;
    if (summaryFilters.rating) {
      const txt = r.ratingKVA != null ? fmt(r.ratingKVA, 0) : '\u2013';
      if (!txt.toLowerCase().includes(summaryFilters.rating.toLowerCase())) return false;
    }
    if (summaryFilters.sizing && !fmt(r.sizingKVA, 1).toLowerCase().includes(summaryFilters.sizing.toLowerCase())) return false;
    if (summaryFilters.active && !fmt(r.activeKVA, 1).toLowerCase().includes(summaryFilters.active.toLowerCase())) return false;
    if (summaryFilters.util) {
      const txt = r.utilizationPct != null ? fmt(r.utilizationPct, 0) + '%' : '\u2014';
      if (!txt.toLowerCase().includes(summaryFilters.util.toLowerCase())) return false;
    }
    return true;
  });

  if (summarySort) equipRows = equipRows.slice().sort((a, b) => summarySortCompare(a, b));

  // Group by system
  const groups = [];
  state.systems.forEach(s => {
    const comps = equipRows.filter(n => n.systemId === s.id);
    if (comps.length) groups.push({ label: s.name, color: s.color, key: 's' + s.id, components: comps });
  });
  const unassigned = equipRows.filter(n => n.systemId == null);
  if (unassigned.length) groups.push({ label: 'Unassigned to system', color: '#5b6675', key: 'unassigned', components: unassigned });

  const body = $('#equipBody'); body.innerHTML = '';
  const ncols = 7;
  groups.forEach(g => {
    const collapsed = collapsedSummaryGroups.has(g.key);
    const head = el('tr', { class: 'system-group-header' });
    const headTd = el('td', { colspan: ncols });
    const caret = el('span', { class: 'collapse-caret' }, [collapsed ? '\u25B8' : '\u25BE']);
    const sw = el('span', { class: 'vl-swatch' }); sw.style.background = g.color;
    const nameSpan = el('span', { style: 'font-weight:800;font-size:12px;letter-spacing:.4px;text-transform:uppercase;padding-left:8px;' }, [g.label]);
    caret.addEventListener('click', (e) => { e.stopPropagation(); if (collapsedSummaryGroups.has(g.key)) collapsedSummaryGroups.delete(g.key); else collapsedSummaryGroups.add(g.key); renderSummary(); });
    headTd.appendChild(caret); headTd.appendChild(sw); headTd.appendChild(nameSpan);
    head.addEventListener('click', () => { if (collapsedSummaryGroups.has(g.key)) collapsedSummaryGroups.delete(g.key); else collapsedSummaryGroups.add(g.key); renderSummary(); });
    head.appendChild(headTd);
    body.appendChild(head);
    if (collapsed) return;
    ...
    g.components.slice().sort((a, b) => ((a.order == null ? 1e9 : a.order) - (b.order == null ? 1e9 : b.order)) || (a.id - b.id))
      .forEach(n => {
        const r = nodeRollup(state.nodes, n, scen);
        const tr = el('tr');
        if (r.utilizationPct != null && r.utilizationPct > 100) tr.classList.add('overloaded');
        tr.appendChild(el('td', { class: 'col-type' }, [el('span', { class: 'node-badge ' + TYPE_META[n.type].badge }, [TYPE_META[n.type].short])]));
        tr.appendChild(el('td', { class: 'col-tag' }, [n.tag || '\u2014']));
        tr.appendChild(el('td', { class: 'col-name' }, [n.name || '\u2014']));
        tr.appendChild(el('td', { class: 'num col-rating' }, [r.ratingKVA != null ? fmt(r.ratingKVA, 0) : '\u2013']));
        tr.appendChild(el('td', { class: 'num col-sizing' }, [fmt(r.sizingKVA, 1)]));
        tr.appendChild(el('td', { class: 'num col-active' }, [fmt(r.activeKVA, 1)]));
        if (r.utilizationPct != null) {
          tr.appendChild(el('td', { class: 'col-util' }, [el('span', { class: 'badge ' + r.badge }, [fmt(r.utilizationPct, 0) + '%'])]));
        } else {
          const td = el('td', { class: 'col-util' }); td.appendChild(el('span', { class: 'badge warn' }, ['\u2014'])); tr.appendChild(td);
        }
        body.appendChild(tr);
      });
  });
  if (hasAnyRating && !equipRows.length) body.appendChild(noResultsRow(ncols));

  // Update sort indicators
  const sths = $all('#equipUtilTable thead tr:first-child th');
  const smap = { 0: 'type', 1: 'tag', 2: 'name', 3: 'rating', 4: 'sizing', 5: 'active', 6: 'util' };
  sths.forEach((th, i) => {
    $all('.sort-ind', th).forEach(s => s.remove());
    if (summarySort && smap[i] === summarySort.key) {
      th.appendChild(el('span', { class: 'sort-ind' }, [summarySort.dir === 'asc' ? '\u25B2' : '\u25BC']));
    }
  });
  applyColumnVisibility('summary', $('#equipUtilTable'));
  stickFilterRow('#equipUtilTable');

  const ol = t.overloaded;
  const hint = $('#overloadHint');
  if (ol.length) hint.textContent = 'Overloaded: ' + ol.map(x => label(x)).join(', ');
...
$('#btnExport').addEventListener('click', () => {
  try {
    const out = { tool: 'capacity-planner', version: 2, state: state };
    const name = state.project.projectNo || state.project.projectName || 'capacity-planner';
...
$('#filein').addEventListener('change', (e) => {
  importJSONFile($('#filein'), {
    validate(data) {
      if (!data || data.tool !== 'capacity-planner') throw new Error('wrong tool');
      if (!data.state || !Array.isArray(data.state.nodes)) throw new Error('missing fields');
    },
    onLoaded(data) {
      state = data.state;
      state.project = Object.assign(defaultState().project, state.project || {});
      state.nextId = (Math.max(0, ...state.nodes.map(x => x.id || 0)) + 1);
      if (!Array.isArray(state.systems)) state.systems = [];
      if (state.nextSystemId == null || !(state.nextSystemId > 0)) {
        state.nextSystemId = Math.max(0, ...state.systems.map(s => s.id || 0)) + 1;
      }
      ...
      if (!Array.isArray(state.scenarios)) { state.scenarios = []; state.activeScenarioId = null; state.nextScenarioId = 1; }
      if (state.nextScenarioId == null || !(state.nextScenarioId > 0)) state.nextScenarioId = Math.max(1, state.scenarios.length + 1);
      if (state.activeScenarioId != null && !state.scenarios.some(s => s.id === state.activeScenarioId)) state.activeScenarioId = null;
      ...
      networkFilters = { search: '', type: '', system: '', status: '', voltage: '', primary: '', redundant: '', feed: '', rating: '', connloads: '', connkw: '', demandkva: '', pf: '', util: '' };
      summaryFilters = { type: '', tag: '', name: '', rating: '', sizing: '', active: '', util: '' };
      netSort = null; schedSort = null; summarySort = null;
...
function coverSectionHTML() {
  ...
  const rows = [
    ...
    ['Operating Scenario', scenarioName()],
  ];
  return '<section class="print-page">' +
    '<div class="cover-title">' + escapeHtml(p.title || 'Capacity Planning Calculation') + '</div>' +
...
function summarySectionHTML() {
  const scen = currentScenario();
  const t = systemTotals(state.nodes, scen);
  const fields = printFieldsHTML([
    ['Connected Load (kW)', fmt(t.connectedKW, 1)], ['Connected Load (kVA)', fmt(t.connectedKVA, 1)],
    ['Demand (kW)', fmt(t.demandKW, 1)], ['Demand (kVA)', fmt(t.demandKVA, 1)],
    ['Utility Demand (kVA)', fmt(t.utilityActive, 1)], ['Utility Contract (kVA)', fmt(t.utilityContract, 0)],
    ['Utility Utilisation', t.utilityPct != null ? fmt(t.utilityPct, 0) + '%' : '\u2014'],
  ]);
  const cats = [
    ['Critical', t.cat.critical.kW, t.cat.critical.kVA],
    ['Essential', t.cat.essential.kW, t.cat.essential.kVA],
    ['Non-essential', t.cat['non-essential'].kW, t.cat['non-essential'].kVA],
  ];
  const catRows = cats.map(c => '<tr><td>' + c[0] + '</td><td>' + fmt(c[1], 1) + '</td><td>' + fmt(c[2], 1) + '</td></tr>').join('');
  return '<section class="print-page">' +
    '<h2 class="print-page-title">Power Summary</h2>' +
    printBlock('Building Totals', fields) +
    printBlock('Demand by Category (active feed)',
      '<table class="print-table"><thead><tr><th>Category</th><th>Demand kW</th><th>Demand kVA</th></tr></thead><tbody>' + catRows + '</tbody></table>') +
    printBlock('Overloaded Equipment',
      t.overloaded.length ? '<div>' + t.overloaded.map(x => escapeHtml(label(x))).join(', ') + '</div>' : '<div>None.</div>') +
    '</section>';
}
